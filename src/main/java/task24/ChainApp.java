...
class Money {
    ...
    public static int setAtm(int atm) {
        if (atm > 0 && atm < 50000 && atm % 50 == 0)
            return atm;
        else {
            throw new IllegalArgumentException("Can't be more then 50_000 or must be multiple 50");
        }
    }

    public static int setAfterTakeMoney(int amount) {
        if (atm > amount)
            return atm -= amount;
        else {
...
abstract class Payments {
    ...
    public int takeMoney(Money money, int count) {
        ...
        } else System.out.println("count must be more then 0 or Atm haven't enough money");
        return result;
    }
